...
def clean_output(output):
    ...
    phrases_to_remove = ["Answer:", "answer:", "Response:", "response:","**Answer:**","**Answer:** ","*","if none"]
...
def create_prompt(subject_entity_id: str,subject_entity: str, relation: str, prompt_templates: dict, instantiated_templates: List[str], tokenizer, few_shot: int = 0, task: str = "fill-mask") -> str:
    ...
    if task == "text-generation":
        if few_shot > 0:
            ...
            # Get relation-specific examples from the dictionary
            relation_specific_examples = instantiated_templates.get(relation, [])
            
            # Randomly sample few-shot examples from the relation-specific examples
            random_examples = random.sample(
                relation_specific_examples, 
                min(few_shot, len(relation_specific_examples))
            )
...
def run(args):
    ...
    if task == "text-generation":
        ...
        if model_type == "Qwen/Qwen2.5-1.5B-Instruct" or model_type == "Qwen/Qwen2.5-0.5B-Instruct":
    ...
    instantiated_templates = {}  # Use a dictionary to store examples by relation
    if task == "text-generation":
        ...
        for row in train_data:
            ...
            if relation not in instantiated_templates:
                instantiated_templates[relation] = []
            instantiated_templates[relation].append(instantiated_example)
    ...
    for row, output, prompt in zip(input_rows, outputs, prompts):
        ...
        if task == "fill-mask":
            ...
            #print("\nresposta final:")
            #print(object_entities_with_wikidata_id)
            #print("\n\n\n")
